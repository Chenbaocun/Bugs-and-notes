...
#### 57. element ui配置了子路由却无法跳转？
- 必须在<el-menu 声明的时候添加router标签。
#### 58. 使用data动态调整css无效？
- style:{'font-size':'450px',color:'red'}//中间带连接线的可以使用‘’方式把它括起来。
#### 59. videoJs突然有一天使用全网址访问不到了，后台显示是匿名用户，改成跨域的api能访问到，不知道为什么之前没事。
